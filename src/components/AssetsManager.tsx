...
const AssetsManager: React.FC<AssetsManagerProps> = ({ authSeed, refreshTrigger, onAssetSelect }) => {
  ...
  const loadAssets = useCallback(async (background = false) => {
    try {
      ...
      if (data.success) {
        ...
        if (!background) {
          showToast(data.message || 'Failed to load assets', 'error');
        }
      }
    } catch (error) {
      if (!background) {
  ...
  const getFileIcon = (mimeType: string) => {
    ...
    if (mimeType === 'application/pdf') return <FileText className="h-5 w-5 text-danger-500" />;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white/80 backdrop-blur-sm p-4 rounded-xl shadow-card border border-neutral-100/80">
        <div className="flex items-center gap-4">
          ...
          {/* View Mode Toggle */}
          <div className="flex items-center border border-neutral-200 rounded-xl overflow-hidden bg-white/80 backdrop-blur-sm">
            <button
      ...
      {selectedAssets.size > 0 && (
        <div className="bg-gradient-to-r from-primary-50 to-primary-100/50 border border-primary-200/50 rounded-xl p-4 backdrop-blur-sm animate-fade-in">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-4 py-2 text-sm font-medium bg-primary-100 text-primary-700 rounded-xl hover:bg-primary-200 transition-all duration-200"
              >
                Select All
              </button>
              <button
                onClick={handleBulkDelete}
                className="px-4 py-2 text-sm font-medium bg-gradient-to-r from-danger-500 to-danger-600 text-white rounded-xl hover:from-danger-600 hover:to-danger-700 transition-all duration-200 shadow-sm hover:shadow-md"
              >
      ...
      <div className="bg-white/80 backdrop-blur-sm rounded-xl shadow-card border border-neutral-100/80 overflow-hidden">
        <div className="p-4 border-b border-neutral-200/60 bg-gradient-to-r from-neutral-50/50 to-neutral-100/30">
        ...
        {assets.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="text-neutral-600 font-medium">No assets found</p>
            {searchTerm && (
              <p className="text-sm text-neutral-400 mt-2">Try adjusting your search terms</p>
            )}
          </div>
        ) : viewMode === 'grid' ? (
          <div className="flex flex-wrap gap-3 p-4">
            {assets.map((asset) => (
              <div 
                key={asset.id} 
                className={`bg-white rounded-xl border border-neutral-200/80 overflow-hidden hover:shadow-md transition-all duration-200 group relative w-[150px] ${
                  ...
                  <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 transition-all duration-200 flex items-center justify-center opacity-0 group-hover:opacity-100">
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="p-2 bg-white bg-opacity-90 text-danger-600 rounded-full hover:bg-opacity-100 transition-colors"
                        title="Delete"
                      >
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="p-2 text-red-400 hover:text-danger-600 transition-colors"
                      title="Delete"
                    >
                      <Trash2 className="h-4 w-4" />
                    </button>
