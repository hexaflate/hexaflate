...
const ProductsView: React.FC<ProductsViewProps> = ({ authSeed }) => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      params.append("limit", limit.toString()); // 0 means show all
  ...
  const handleCSVExport = async () => {
    try {
      ...
      setExportErrorMessage(
        "Gagal mengekspor data: " + (error as Error).message,
      );
  ...
  const groupedProducts = products.reduce(
    (
      groups: Record<string, { products: Product[]; operatorName: string }>,
      product: Product,
    ) => {
      ...
      if (!groups[operatorCode]) {
        ...
        groups[operatorCode] = {
          products: [],
          operatorName: product.operator_nama || operatorCode,
        };
      }
  ...
  const sortedGroupedProducts = Object.entries(groupedProducts).map(
    ([operatorCode, group]) => ({
      operatorCode,
      group,
    }),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white/80 backdrop-blur-sm rounded-xl shadow-card p-6 border border-neutral-100/80">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          <select
            ...
            className="block w-full px-4 py-2.5 bg-white/80 backdrop-blur-sm border border-neutral-200/80 rounded-xl focus:ring-2 focus:ring-success-500/20 focus:border-success-400 transition-all duration-200"
          >
        ...
        <div className="flex items-center justify-end">
          <div className="flex items-center space-x-2">
            ...
            <select
              ...
              className="px-3 py-1.5 bg-white/80 border border-neutral-200/80 rounded-lg text-sm focus:ring-2 focus:ring-success-500/20 focus:border-success-400 transition-all duration-200"
            >
      ...
      <div className="bg-white/80 backdrop-blur-sm rounded-xl shadow-card overflow-hidden border border-neutral-100/80">
        <div className="px-6 py-4 border-b border-neutral-100 flex items-center justify-between bg-gradient-to-r from-neutral-50/50 to-white">
          ...
          <button
            ...
            className="flex items-center px-4 py-2 bg-gradient-to-r from-success-500 to-success-600 text-white rounded-xl hover:from-success-600 hover:to-success-700 focus:ring-2 focus:ring-success-500/30 focus:ring-offset-2 transition-all duration-200 font-medium shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isExportingCSV ? (
              <>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {sortedGroupedProducts.map(({ operatorCode, group }) => {
                  ...
                  const operatorData = operators.find(
                    (op) => op.kode === operatorCode,
                  );
                  const hasOperatorIssue =
                    operatorData &&
                    (operatorData.gangguan === 1 || operatorData.kosong === 1);
                  ...
                  return (
                    <React.Fragment key={operatorCode}>
                      ...
                      <tr
                        className={
                          hasOperatorIssue ? "bg-red-200" : "bg-gray-100"
                        }
                      >
                        <td
                          colSpan={3}
                          className={`px-6 py-3 text-sm font-semibold text-center ${hasOperatorIssue ? "text-red-900" : "text-gray-900"}`}
                        >
                          {group.operatorName}
                        </td>
                      </tr>
                      ...
                      {group.products.map((product: Product) => {
                        const hasProductIssue =
                          product.gangguan === 1 || product.kosong === 1;
                        // Use operator issue styling if operator has issues, otherwise use product-specific styling
                        const shouldUseRedStyling =
                          hasOperatorIssue || hasProductIssue;

                        return (
                          <tr
                            key={product.kode}
                            className={`${shouldUseRedStyling ? "bg-danger-50 hover:bg-danger-100" : "hover:bg-gray-50"}`}
                          >
                            <td
                              className={`px-6 py-4 whitespace-nowrap text-sm font-medium ${shouldUseRedStyling ? "text-red-900" : "text-gray-900"}`}
                            >
                              {product.kode}
                            </td>
                            <td
                              className={`px-6 py-4 text-sm max-w-xs ${shouldUseRedStyling ? "text-red-900" : "text-gray-900"}`}
                            >
                              <div className="flex items-center space-x-2">
                                <div
                                ...
                                <div className="flex items-center space-x-1 flex-shrink-0">
                                  ...
                                  {product.kosong === 1 && (
                                    <div
                                      className="flex items-center justify-center w-5 h-5 bg-danger-100 rounded-full"
                                      title="Kosong"
                                    >
                            ...
                            <td
                              className={`px-6 py-4 whitespace-nowrap text-right text-sm font-medium ${shouldUseRedStyling ? "text-danger-600" : "text-success-600"}`}
                            >
