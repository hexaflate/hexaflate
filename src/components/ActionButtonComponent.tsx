...
const ActionButtonComponent: React.FC<ActionButtonComponentProps> = ({
  ...
  const getVariantClasses = () => {
    ...
    if (hasValidationErrors && showValidationErrors) {
      return `${baseClasses} bg-danger-100 border-2 border-danger-300 text-danger-600 hover:bg-danger-200`;
    }
    ...
    switch (variant) {
      ...
      default:
        return `${baseClasses} text-white shadow-sm hover:shadow-md`;
    }
  };

  const getVariantStyle = (): React.CSSProperties | undefined => {
    if (hasValidationErrors && showValidationErrors) return undefined;
    switch (variant) {
      case 'outline':
        return { color: THEME_COLOR, borderColor: THEME_COLOR };
      default:
        return { backgroundColor: THEME_COLOR };
    }
  };
