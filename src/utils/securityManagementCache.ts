...
export function setCachedSecurityManagement(cache: Partial<SecurityManagementCache>): void {
  try {
    ...
    const entry: SecurityManagementCache = {
      ...
      loginConfig: cache.loginConfig !== undefined ? cache.loginConfig : (existing?.loginConfig ?? null),
      timestamp: Date.now(),
    };
...
export function preloadSecurityManagement(authSeed: string): Promise<SecurityManagementCache | null> {
  return new Promise(async (resolve) => {
    try {
      ...
      const [configRes, appRulesRes, cutoffRes, demoRes, loginRes] = await Promise.all([
        ...
        fetch(await getApiUrl('/admin/login-config'), {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'X-Token': X_TOKEN_VALUE,
      ...
      const [configData, appRulesData, cutoffData, demoData, loginData] = await Promise.all([
        configRes.json(),
        appRulesRes.json(),
        cutoffRes.json(),
        demoRes.json(),
      ...
      const cache: SecurityManagementCache = {
        ...
        loginConfig: loginData.success && loginData.config ? loginData.config : null,
        timestamp: Date.now(),
      };
